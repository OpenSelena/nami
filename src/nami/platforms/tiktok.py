"""TikTok platform adapter for Nami."""
...
class TikTokAdapter(BasePlatformAdapter):
    ...
    def download_photos(
        ...
        with ArchiveLock(photos_dir):
            rc, failure_type, output = execute_with_intelligent_retry(
                attempt, cookies_arg, log_file, "gallery-dl (tiktok photos)"
            )
    ...
    def download_videos(
        ...
        with ArchiveLock(videos_dir):
            rc, failure_type, output = execute_with_intelligent_retry(
                attempt_gd, cookies_arg, log_file, "gallery-dl (tiktok videos)"
            )
            ...
            if rc != 0:
                ...
                yt_rc, yt_failure, yt_output = execute_with_intelligent_retry(
                    attempt_yt, cookies_arg, log_file, "yt-dlp (tiktok videos)"
                )
                rc = yt_rc
    ...
    def download_stories(
        ...
        return DownloadResult(status=DownloadResultStatus.UNSUPPORTED, message="Stories not supported for TikTok")
    ...
    def download_highlights(
        ...
        return DownloadResult(status=DownloadResultStatus.UNSUPPORTED, message="Highlights not supported for TikTok")
